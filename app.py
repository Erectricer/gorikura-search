...
def load_member_data():
    ...
    try:
        ...
        for _, row in df.iterrows():
            try:
                # データの位置がズレたり空っぽでもエラーにしない対策
                if len(row) < 5:
                    continue
                name = str(row.iloc[1]).strip() if pd.notna(row.iloc[1]) else ""
                tag = str(row.iloc[2]).strip() if pd.notna(row.iloc[2]) else "なし"
                season = str(row.iloc[3]).strip() if pd.notna(row.iloc[3]) else "不明"
                nicknames_raw = str(row.iloc[4]).strip() if pd.notna(row.iloc[4]) else ""
        ...
        # スプレッドシートの読み込み自体が失敗しても、システムは止めない
        pass
    return database
...
with tab1:
    ...
    # データが空っぽでも、検索窓自体は【絶対に強制表示】する
    search_member = st.text_input("検索したいメンバー名・タグを入力してね：", placeholder="例: チョコビ、saba919919 など")

    if search_member:
        if not member_database:
            st.error("❌ 現在、スプレッドシートのデータを読み込み中です。少し待ってから再度検索してください。")
        else:
            found_member = None
            found_name = ""
